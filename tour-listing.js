...
function renderTourCards(containerId, options = {}) {
  ...
  filteredTours.forEach((tour) => {
      ...
      if (tour.type === "self-guided") {
        ...
        detailsHTML = `
          <div class="card-details">
            ...
            <span class="material-symbols-sharp" aria-hidden="true">person</span> ${tour.guestMin}-${tour.guestMax}
            <br>
            <span style="font-weight:bold;">${tourPrice}</span>${(tour.status.isActive && !tour.status.isCurrentlyRunning) ? '&ensp;•&ensp;<span class="seasonal-label">Seasonal</span>' : ''}
          </div>`;
      }
    } else {
      detailsHTML = `
    ...
    const kidFriendlyBadgeHTML = tour.kidFriendly ? '<span class="kid-friendly-badge"><span class="material-symbols-sharp" aria-hidden="true">child_care</span> Great for Kids</span>' : '';
    const newTourPrefixHTML = tour.newTour ? '<em>New Tour!</em><br>' : '';
